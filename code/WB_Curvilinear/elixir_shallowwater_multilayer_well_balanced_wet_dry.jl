...
function initial_condition_well_balancedness(x, t,
    ...
    x1, x2 = x
    b = (2.0 / exp(0.5 * ((x1 - 1.0)^2 + (x2 - 1.0)^2)) +
         2.0 / exp(0.5 * ((x1 + 1.0)^2 + (x2 + 1.0)^2)))
...
###############################################################################
# Calculate additional lake-at-rest errors for all layers
@inline function lake_at_rest_error_H1(u,
                                       equations::ShallowWaterMultiLayerEquations2D)
...
@inline function lake_at_rest_error_H2(u,
                                       equations::ShallowWaterMultiLayerEquations2D)
    h = Trixi.waterheight(u, equations)
    b = u[end]
    ...
    # can emerge out of the water as well as for the threshold offset to avoid
    # division by a "hard" zero water heights as well.
    H0_wet_dry = max(1.0, b + equations.threshold_limiter)

    return abs(H0_wet_dry - (sum(h[2:end]) + b))
end

@inline function lake_at_rest_error_H3(u,
                                       equations::ShallowWaterMultiLayerEquations2D)
    h = Trixi.waterheight(u, equations)
    b = u[end]
    ...
    # can emerge out of the water as well as for the threshold offset to avoid
    # division by a "hard" zero water heights as well.
    H0_wet_dry = max(0.5, b + equations.threshold_limiter)

    return abs(H0_wet_dry - (sum(h[3:end]) + b))
end
...
analysis_callback = AnalysisCallback(semi, interval = analysis_interval,
                                     ...
                                     extra_analysis_integrals = (lake_at_rest_error_H1,
                                                                 lake_at_rest_error_H2,
                                                                 lake_at_rest_error_H3),
